...
router.get("/user/:rfid_tag", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ error: "Error al obtener las sesiones del usuario" });
  }
});
...
router.get("/average", async (req, res) => {
  ...
  if (!rfid_tag || !machine_id) {
    return res
      .status(400)
      .json({ error: "rfid_tag y machine_id son requeridos" });
  }
  ...
  try {
    ...
    if (result.rows[0].average_time_seconds === null) {
      return res
        .status(404)
        .json({ error: "No hay datos suficientes para calcular la media" });
    }
